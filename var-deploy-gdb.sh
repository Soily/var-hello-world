...
readonly PROGRAM="$2"
readonly TARGET_DIR="/home/soilwork"

# Must match startsPattern in tasks.json
echo "Deploying to target"

# kill gdbserver on target and delete old binary
ssh soilwork@${TARGET_IP} "sh -c '/usr/bin/killall -q gdbserver; rm -rf ${TARGET_DIR}/${PROGRAM}  exit 0'"

# send the program to the target
scp ${PROGRAM} soilwork@${TARGET_IP}:${TARGET_DIR}
